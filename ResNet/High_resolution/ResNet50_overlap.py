...
###################################
    ...
    def block3_head(self,x):
        ...
        identity = x #28 
        # identity1 = identity[:,:,-1:,:].clone()
        identity =self.block3_downsample(identity) #28
        out = self.block3_conv1_1(x)
        out = self.block3_bn1_1(out) #28
        out = self.relu(out) 
        # bound1 = out[:,:,-2:,:].clone()
        ...
        identity = out
        out = self.block3_conv3_1(out)#26
        ...
        identity = out
        out = self.block3_conv4_1(out)#26
        ...
        identity = out
        out = self.block3_conv5_1(out)#26
        ...
        identity = out
        out = self.block3_conv6_1(out)#26
        ...
        identity = out #28 
        # identity1 = identity[:,:,-1:,:].clone()
        identity =self.block4_downsample(identity) #28
        out = self.block4_conv1_1(out)
        out = self.block4_bn1_1(out) #28
        out = self.relu(out) 
        # bound1 = out[:,:,-2:,:].clone()
        ...
        identity = out
        out = self.block4_conv3_1(out)#26
    ...
    def block3_forward(self,x):
        ...
        outs =[checkpoint(self.block3_head,xs[0])]

        for x in xs[1:-1]:
            # print(x.size())
            x.requires_grad_(True)
            out = checkpoint(self.block3_medium,x)
            # print(out.size())
            outs.append(out)
        xs[-1].requires_grad_(True)
        outs.append( checkpoint(self.block3_tail,xs[-1]))
    ...
    def forward(self,x):
        ...
        # x8 = x[:,:,7*H-3:,:]
        xs[0].requires_grad_(True)
        outs =[checkpoint(self.head,xs[0])]

        for x in xs[1:-1]:
            # print(x.size())
            x.requires_grad_(True)
            out = checkpoint(self.medium,x)
            # print(out.size())
            outs.append(out)
        xs[-1].requires_grad_(True)
        outs.append( checkpoint(self.tail,xs[-1]))
