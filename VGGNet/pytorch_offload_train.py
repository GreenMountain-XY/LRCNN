...
# from VGG16_4t6 import VGG16
from VGG16 import VGG16
# from VGG16_10 import VGG16
from load_ImageNet import load_ImageNet


train_loader, val_loader, train_dataset, val_dataset = load_ImageNet('dataset',batch_size=256)
torch.cuda.empty_cache()
import time
t0 = time.time()  
# net = Net(offload_ratio=1) 
net = VGG16(num_classes=10,num_channels=84)
...
for epoch in range(5):  # loop over the dataset multiple times 训练5轮
    ...
    # test_accuracys.append(test_correct/test_num)
        # print(f"整个训练过程显存最大占用:{torch.cuda.max_memory_allocated()/(1024 * 1024 * 1024):.2f}GB")
t1 = time.time()
print(t1-t0)
print(f"整个训练过程显存最大占用:{torch.cuda.max_memory_allocated()/(1024 * 1024 * 1024):.2f}GB")
del(net)
