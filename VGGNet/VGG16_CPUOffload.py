...
class Net(nn.Module):
    def __init__(self,  offload_ratio= 0,num_channels=64):
        super(Net, self).__init__()
        self.model = VGG16(num_classes=10,num_channels= num_channels)
        self.offload_ratio = offload_ratio
    def forward(self, x):
        with CPUOffload(offload_ratio=self.offload_ratio, tag="vgg16"):
            x = self.model(x)
        return x